...
std::pair<float, float> Mouse::GetCorrectedPos(int windowWidth, int windowHeight) const noexcept
{
	float correctedX = float(x) - float(windowWidth) / 2.0f;
	float correctedY = float(y) - float(windowHeight) / 2.0f;
...
int Mouse::GetWheelDeltaCarry(void) const noexcept
{
	return wheelDeltaCarry;
}


void Mouse::SetWheelDeltaCarry(int i) noexcept
{
	wheelDeltaCarry = i;
}